...
def test_fail():
    runner = CliRunner()
    result = runner.invoke(cli, ['bounds'], '5')
    assert result.exit_code == 1


def test_bounds():
    runner = CliRunner()
    result = runner.invoke(cli, ['bounds'], input, catch_exceptions=False)
...
def test_bounds_explode():
    runner = CliRunner()
    result = runner.invoke(cli, ['bounds', '--explode'], input)
    assert result.exit_code == 0
    assert len(json.loads(result.output.strip())) == 4
    assert round(json.loads(result.output.strip())[0], 1) ==  81819.8
...
def test_bounds_with_id():
    runner = CliRunner()
    result = runner.invoke(cli, ['bounds', '--with-id'], input)
    assert result.exit_code == 0
...
def test_bounds_explode_with_id():
    runner = CliRunner()
    result = runner.invoke(cli, ['bounds', '--explode', '--with-id'], input)
    assert result.exit_code == 0
    obj = json.loads(result.output.strip())
    assert 'id' in obj
    assert 'bbox' in obj
...
def test_bounds_with_obj():
    runner = CliRunner()
    result = runner.invoke(cli, ['bounds', '--with-obj'], input)
    assert result.exit_code == 0
    obj = json.loads(result.output.strip())
...
def test_bounds_explode_with_obj():
    runner = CliRunner()
    result = runner.invoke(cli, ['bounds', '--explode', '--with-obj'], input)
    assert result.exit_code == 0
    obj = json.loads(result.output.strip())
    assert 'geometry' in obj
    assert 'id' in obj
