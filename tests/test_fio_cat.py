...
def test_cat_uk():
    runner = CliRunner()
    result = runner.invoke(cli, ['cat', 'docs/data/test_uk.shp'])
    assert result.exit_code == 0
...
def test_collect_ld():
    ...
    result = runner.invoke(
        cli,
        ['collect', '--with-ld-context', '--add-ld-context-item', 'foo=bar'],
        input,
        catch_exceptions=False)
    assert result.exit_code == 0
...
def test_collect_rec_buffered():
    runner = CliRunner()
    result = runner.invoke(cli, ['collect', '--record-buffered'], input)
    assert result.exit_code == 0
...
def test_distrib():
    runner = CliRunner()
    result = runner.invoke(cli, ['distrib'], input)
    assert result.exit_code == 0
...
def test_distrib():
    runner = CliRunner()
    result = runner.invoke(cli, ['distrib'], input_collection)
    assert result.exit_code == 0
...
def test_dump():
    runner = CliRunner()
    result = runner.invoke(cli, ['dump', 'docs/data/test_uk.shp'])
    assert result.exit_code == 0
    assert '"FeatureCollection"' in result.output
    assert result.output.count('"Feature"') == 48
